...
import {Button, Form, InputGroup, Badge, ListGroup, ButtonGroup} from "react-bootstrap";
...
function MainCommunity() {
    ...
    const [sortCriteria, setSortCriteria] = useState("latest"); // 정렬 기준 추가
    ...
    // 정렬 함수
    const sortBoards = (criteria) => {
        setSortCriteria(criteria); // 선택된 정렬 기준 설정

        let sortedBoards = [...filteredBoards];
        switch (criteria) {
            case "likes":
                sortedBoards.sort((a, b) => b.boardLikes - a.boardLikes); // 좋아요 수 내림차순
                break;
            case "views":
                sortedBoards.sort((a, b) => b.boardView - a.boardView); // 조회수 내림차순
                break;
            case "latest":
            default:
                sortedBoards.sort((a, b) => new Date(b.boardCreatedAt) - new Date(a.boardCreatedAt)); // 최신순
                break;
        }
        setFilteredBoards(sortedBoards); // 정렬된 결과 업데이트
    };
    ...
    return (
        <div className={`main-container ${theme}`}>
            <MainHeader/>
            <div className="row g-5" style={{marginTop:"70px", marginLeft:"20px", marginRight:"20px"}}>
                ...
                <div className="col-md-4">
                    ...
                    <div className="p-4 mt-3 rounded"
                        ...
                        <ListGroup variant="flush">
                            ...
                            >
                                <div style={{fontWeight: 'bold', color:'black'}}>{board.boardTitle}</div>
